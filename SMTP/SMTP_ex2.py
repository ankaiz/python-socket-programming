...
# Send message data.
# Fill in start
subject = "Subject: SMTP EX2 mail client testing\r\n\r\n"
clientSocketSSL.send(subject.encode())
clientSocketSSL.send(msg.encode())
# Fill in end

##read img
with open('earth.jpeg', 'rb') as f:
    img = base64.b64encode(f.read())
imgmsg = 'Content-Type:multipart/related; boundary="----=_NextPart_000_0012345JZ"\r\n'.encode()
imgmsg += 'MIME-Version: 1.0\r\n'.encode()
imgmsg += 'Content-Type: image/jpeg; name="earth.jpeg"\r\n'.encode()
imgmsg += 'Content-Transfer-Encoding: base64\r\n'.encode()
imgmsg += 'Content-ID: <earth.jpeg>'.encode()
imgmsg += '\r\n'.encode()
imgmsg += img + "\r\n".encode()
imgmsg += '\r\n'.encode()
clientSocketSSL.send(imgmsg)
##
